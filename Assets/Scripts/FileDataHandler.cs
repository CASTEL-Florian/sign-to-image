...
public class FileDataHandler
{
    ...
    public PlayerInfos PlayerLoad()
    {
        // using Path.Combine to account for different OS's having different path separator
        string fullPath = Path.Combine(dataDirPath, dataFileName);
        PlayerInfos loadedData = new PlayerInfos();
        if (File.Exists(fullPath))
        {
            if (dataInputField)
                dataInputField.text = "ya un de fichier";
            try
            {
                ...
                // deserialize the data from Json back into the c# object
                loadedData = JsonUtility.FromJson<PlayerInfos>(dataToLoad);
            }
            catch (Exception ex)
            {
                Debug.LogError("Error occured when trying to load data to file: " + fullPath + "\n" + ex);
    ...
    public void Save(PlayerInfos data)
    {
        // using Path.Combine to account for different OS's having different path separator
        string fullPath = Path.Combine(dataDirPath, dataFileName);
        try
